...
class  InputTree
{
 public:
  ...
  InputTree() : parent_(NULL), start_(0), finish_(0), word_(""), term_("") {}
  InputTree(int s, int f, const ECString w, const ECString t, const ECString n,
	    InputTrees& subT, InputTree* par, InputTree* headTr)
    : parent_(par), start_(s), finish_(f), num_(-1), word_(w), term_(t), 
      ntInfo_(n), headTree_(headTr), subTrees_(subT){}
  InputTree(const ECString w, int i)
    : parent_(NULL), start_(i), finish_(i+1), num_(-1), word_(w), term_(w), 
      ntInfo_(""), headTree_(NULL){}
  ~InputTree();
  ...
  /** Converts a Charniak tree to Tree */
  srl::TreePtr convertToTree();
