...
Edge::
Edge(ConstTerm* trm)
  : num_(-1), status_(0), start_(-1), loc_( -1 ), lhs_( trm ), 
    finishedParent_( NULL ), item_(NULL), pred_(NULL), 
    heapPos_(-1), demerits_(0),
...
: num_(-1), status_(right), start_(src.start_), loc_( src.loc_ ), 
  lhs_( src.lhs_ ), finishedParent_( NULL ), item_(&itm),
  heapPos_(-1),
...
Edge::
Edge( Item& itm )
: num_(-1), status_(2), start_(itm.start()), loc_( itm.finish() ), 
  lhs_( itm.term() ),  finishedParent_( &itm ), item_(NULL),
  pred_(NULL), heapPos_(-1), 
  demerits_(0),
  leftMerit_(1),
  rightMerit_(1),
  prob_( itm.prob() )
