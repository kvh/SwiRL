...
{
 public:
  FTypeTree() : n(-1), left(NULL), right(NULL), back(NULL) {}
  FTypeTree(int fi) : n(fi), left(NULL), right(NULL), back(NULL) {}
  int n;
  FTypeTree* left;
  FTypeTree* right;
  FTypeTree* back;
...
class SubFeature
{
 public:
  SubFeature(int i, ECString& nm, int fnn, list<int> fl)
    : num(i), name(nm), fun(Funs[fnn]), usf(fnn), featList(fl) {}
...
class Feature
{
 public:
  Feature(int i, ECString& nm, int ff, int pos, int cpr)
    : num(i), name(nm), subFeat(ff), startPos(pos), condPR(cpr), auxCnt(0) {}
